...
Demo `FMX` para validar opciones del nuevo `TGMLibMap` antes y después de activar el mapa.
...
- `GestureHandling`
- `MapTypeControl`, `ZoomControl`, `StreetViewControl` y `FullscreenControl`
- posiciones de controles visibles
- opciones de inicialización como `ColorScheme`, `ControlSize`, `MapId` y `RenderingType`
- recepción de eventos principales del mapa, incluido `click` enriquecido con
  `placeId`, `contextmenu`, `heading_changed`, `tilt_changed`, `mouseover`,
  `mouseout`, `projection_changed` y `renderingtype_changed`

## Uso

1. Abrir `GMLibMapOptionsLabFmx.dproj`.
