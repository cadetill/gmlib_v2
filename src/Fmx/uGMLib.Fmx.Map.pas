{**
  ...
  Esta unidad define la primera especializaciÃ³n `FMX` de `TGMCustomMap`.
}
unit uGMLib.Fmx.Map;
...
type
  ...
  private
    FBrowser: TComponent;
    FBridgeImpl: IGMBridgeTransport;
    ...
    function CreateBridgeForBrowser(const ABrowser: TComponent): IGMBridgeTransport;
  ...
  public
    ...
    {** @abstract(Activa el mapa asegurando que `Browser` y bridge estÃ©n preparados.) }
...
function TGMLibFmxMap.CreateBridgeForBrowser(
  const ABrowser: TComponent): IGMBridgeTransport;
begin
