...
type
  {** @abstract(Implementación del transporte CEF para FMX.) }
  TGMLibFmxCefBridge = class(TInterfacedObject, IGMBridgeTransport)
  private const
...
procedure TGMLibFmxCefBridge.PostCommand(const AEnvelope: TGMMessageEnvelope);
begin
  ExecuteJavaScript(Format('window.gmlib.receiveCommand(%s);', [AEnvelope.ToJson]));
end;

procedure TGMLibFmxCefBridge.SetOnMessageReceived(const AHandler: TGMBridgeMessageReceivedEvent);
begin
