{**
  @abstract(Soporte de carga y composición del bootstrap VCL del mapa.)
  @author(Xavier Martinez (cadetill) <cadetill@gmail.com>)

  Esta unidad centraliza la composición del bootstrap HTML/JavaScript del mapa
  para VCL, materializando los recursos embebidos en una carpeta temporal y
  usando recursos de disco solo como fallback de desarrollo.
}
unit uGMLib.Vcl.MapBootstrap;
...
type
  {** @abstract(Helper para construir el HTML de arranque del mapa VCL.) }
  TGMLibMapBootstrap = class
  private
...
class function TGMLibMapBootstrap.BuildHtml(AMap: TGMCustomMap): string;
var
...
class function TGMLibMapBootstrap.BuildMapsApiScriptUrl(AMap: TGMCustomMap): string;
var
  ChannelParam: string;
  MapIdParam: string;
begin
...
class function TGMLibMapBootstrap.BuildMapsApiChannelParam(AMap: TGMCustomMap): string;
var
  ChannelValue: string;
begin
...
class function TGMLibMapBootstrap.BuildTemplateFallback: string;
begin
  Result :=
    '<!DOCTYPE html>' +
    '<html lang="en">' +
