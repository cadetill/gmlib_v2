...
{$ENDIF}
    ...
    procedure HandleBridgeMessage(Sender: TObject; const AEnvelope: TMapLibMessageEnvelope);
...
function TGMCustomMapLinkedComponent.GetDocumentationUrl: string;
begin
...
procedure TGMCustomMapLinkedComponent.ProcessMapMessage(
  const AEnvelope: TMapLibMessageEnvelope);
begin
end;
...
function TGMCustomMap.GetDocumentationUrl: string;
begin
...
procedure TGMCustomMap.ProcessIncomingMessage(const AEnvelope: TMapLibMessageEnvelope);
var
...
{$ENDIF}
  ...
  begin
    if Assigned(FLayers) then
{$IFDEF FPC}
      FLayers.Kml.SetStatus(MapLibMessageEnvelopePayloadAsString(AEnvelope));
{$ELSE}
...
{$ENDIF}
  ...
  begin
{$IFDEF FPC}
    NewZoom := StrToIntDef(MapLibMessageEnvelopePayloadAsString(AEnvelope), FOptions.Zoom);
{$ELSE}
...
{$ENDIF}
  ...
  begin
{$IFDEF FPC}
    if TryParseMapTypeId(MapLibMessageEnvelopePayloadAsString(AEnvelope), NewMapTypeId) then
{$ELSE}
...
{$ENDIF}
  ...
  begin
{$IFDEF FPC}
    NewHeading := StrToFloatDef(MapLibMessageEnvelopePayloadAsString(AEnvelope), FOptions.Heading, GMLibInvariantFormatSettings);
{$ELSE}
...
{$ENDIF}
  ...
  begin
{$IFDEF FPC}
    NewTilt := StrToIntDef(MapLibMessageEnvelopePayloadAsString(AEnvelope), FOptions.Tilt);
{$ELSE}
...
{$ENDIF}
  ...
  begin
{$IFDEF FPC}
    if TryParseRenderingType(MapLibMessageEnvelopePayloadAsString(AEnvelope), NewRenderingType) then
{$ELSE}
...
procedure TGMCustomMap.SetBridge(const Value: IMapBridgeTransport);
begin
