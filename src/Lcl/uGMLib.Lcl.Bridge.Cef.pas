...
type
  {** @abstract(Implementación del transporte CEF para LCL.) }
  TGMLibLclCefBridge = class(TInterfacedObject, IGMBridgeTransport)
  private const
...
begin
  ...
  try
    Envelope := GMLibMessageEnvelopeFromJson(AMessage);
  except
    Exit;
  end;
...
procedure TGMLibLclCefBridge.PostCommand(const AEnvelope: TGMMessageEnvelope);
begin
  {$IFDEF FPC}
  ExecuteJavaScript(Format('window.gmlib.receiveCommand(%s);', [GMLibMessageEnvelopeToJson(AEnvelope)]));
  {$ELSE}
  ExecuteJavaScript(Format('window.gmlib.receiveCommand(%s);', [AEnvelope.ToJson]));
  {$ENDIF}
end;

procedure TGMLibLclCefBridge.SetOnMessageReceived(const AHandler: TGMBridgeMessageReceivedEvent);
begin
